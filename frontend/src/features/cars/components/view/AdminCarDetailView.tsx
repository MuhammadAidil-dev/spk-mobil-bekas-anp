...
import { ArrowLeft, Edit, Fuel, Gauge, Settings, Users, MapPin, Calendar, Tag, Palette } from 'lucide-react';
import { getCarImageUrl } from '@/lib/api';
import type { ApiCar } from '@/types/api.type';

interface AdminCarDetailViewProps {
  car: ApiCar;
}
...
export default function AdminCarDetailView({ car }: AdminCarDetailViewProps) {
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-3">
          <Link href="/admin/cars" className="rounded-lg p-2 text-secondary transition hover:bg-slate-100">
            <ArrowLeft size={20} />
          </Link>
          <div>
        ...
        <div className="col-span-12 space-y-6 lg:col-span-8">
          <section className="rounded-2xl border border-slate-200 bg-white p-6">
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
              ...
              <SpecCard icon={<Gauge size={18} />} label="Kilometer" value={`${car.mileage.toLocaleString('id-ID')} km`} />
              <SpecCard icon={<Settings size={18} />} label="Kapasitas Mesin" value={`${car.engine_capacity} cc`} />
              <SpecCard icon={<Users size={18} />} label="Kapasitas Kursi" value={`${car.seat_capacity} orang`} />
              <SpecCard
              ...
              />
              <SpecCard icon={<Palette size={18} />} label="Warna" value={car.color} />
              {car.plate_region && (
                <SpecCard icon={<MapPin size={18} />} label="Plat Wilayah" value={car.plate_region} />
              )}
            </div>
          </section>
