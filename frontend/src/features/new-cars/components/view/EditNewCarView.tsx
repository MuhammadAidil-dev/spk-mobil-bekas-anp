...
import { updateNewCarAction } from '@/actions/new-cars.action';
...
interface EditNewCarViewProps {
  car: ApiNewCar;
}

export default function EditNewCarView({ car }: EditNewCarViewProps) {
  ...
  const breadcrumbItems: breadcrumbItemsType[] = [
    ...
    { label: `${car.brand} ${car.model}`, href: `/admin/new-cars/${car._id}` },
    { label: 'Edit' },
  ];
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await updateNewCarAction(car._id, formData);
      if (result.success) {
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <div className="mx-auto w-full max-w-7xl flex-1 py-8">
        ...
        <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">Edit Mobil Baru</h1>
        ...
        {/* Form */}
        <form id="edit-new-car-form" ref={formRef} onSubmit={handleSubmit}>
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                  <Field label="Engine Capacity (CC)">
                    <input
                      ...
                      type="number"
                      defaultValue={car.engine_capacity}
                      min={1}
                      required
                      className="w-full rounded-xl border border-slate-300 px-4 py-3 outline-none focus:border-primary"
                    />
                  ...
                  <Field label="Fuel Efficiency (L/100km)">
                    <input
                      ...
                      type="number"
                      defaultValue={car.fuel_efficiency}
                      step="0.1"
                      min={0.1}
                      required
                      className="w-full rounded-xl border border-slate-300 px-4 py-3 outline-none focus:border-primary"
              ...
              <section className="rounded-2xl border border-slate-200 bg-white p-6">
                ...
                {car.image_url && !imagePreview && (
                  <div className="mb-4 overflow-hidden rounded-xl border border-slate-200">
                    ...
                    />
                    <p className="bg-slate-50 px-3 py-1.5 text-center text-xs text-slate-400">
                      Foto saat ini
                    </p>
                  </div>
                )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-2xl">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <button
                onClick={() => router.push(`/admin/new-cars/${car._id}`)}
              ...
              <button
                onClick={() => { router.push('/admin/new-cars'); router.refresh(); }}
