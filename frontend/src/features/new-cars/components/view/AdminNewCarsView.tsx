...
import { deleteNewCarAction } from '@/actions/new-cars.action';
import { getCarImageUrl } from '@/lib/api';
import type { ApiNewCar } from '@/types/api.type';
import ConfirmModal from '@/components/ui/ConfirmModal';

interface AdminNewCarsViewProps {
  cars: ApiNewCar[];
}
...
export default function AdminNewCarsView({ cars }: AdminNewCarsViewProps) {
  ...
  function requestDelete(car: ApiNewCar) {
    setTargetCar(car);
      ...
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground sm:text-3xl">
            Mobil Baru Inventori Manajemen
          </h1>
          <p className="mt-1 text-sm text-secondary">
            Kelola data mobil baru untuk proses perhitungan SPK ANP.
          </p>
        </div>
      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
          ...
          <table className="min-w-150">
            <thead className="bg-neutral">
              <tr className="text-left text-xs uppercase tracking-wider text-secondary">
                ...
                <th className="px-4 py-3 sm:px-6 sm:py-4">Efisiensi BBM</th>
        ...
        <div className="flex items-center justify-between border-t border-slate-200 bg-white px-4 py-3 sm:px-6 sm:py-4">
          <p className="text-sm text-secondary">
            Menampilkan {filtered.length} dari {cars.length} mobil baru
          </p>
        </div>
      </section>
